...
export const PaymentStats = ({ gymId }: { gymId?: string }) => {
    const theme = useTheme()
    const iconStyle = { color: theme.palette.primary.main, fontSize: 40 }
    const { data, isLoading } = usePaymentsStats(gymId)

    const totalMontoHoy = data?.montoHoy ?? 0
    const totalPagosHoy = data?.pagosHoy ?? 0
    ...
    return (
        ...
        >
            <StatCard
                ...
                value={isLoading ? '—' : `$ ${totalMontoHoy.toLocaleString('es-AR')}`}
                    ...
                    <Box display="flex" flexDirection="column" width="100%" mt={1}>
                        ...
                        <Box display="flex" height={10} borderRadius={5} overflow="hidden">
                            {byMethod.map((m: any, index: number) => {
                                const percent = Math.round((m.count * 100) / (totalPagosHoy || 1))
                                return (
                                    <Tooltip
                                        key={m.metodo}
